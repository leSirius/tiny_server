...
public:
    ...
    LoggerImpl(LogLevel _level, const SourceFile &source, int _line);
...
Logger::Logger(LogLevel _level, const string_view _file, int _line): impl(
    make_unique<LoggerImpl>(_level, SourceFile(_file), _line)) {
}
...
LoggerImpl::LoggerImpl(const LogLevel _level, const SourceFile &source, const int _line)
    : sourceFile(source), line(_line), level(_level) {
    formattedTime();
    stream << "TID[" << tcp::currentThread::tidString() << "]  ";
...
void LoggerImpl::finish() { stream << "  -  " << sourceFile.fileName << ":" << line << "\n"; }
